...
第一次成功发送消息后，Companion 可以请求浏览器通知权限。授权后，已打开但不可见的页面会为每个新完成的 Partner turn 提醒一次。通知只显示 Partner 名字和通用的新消息提示，不包含回复正文。这是页面本地通知，不是 Web Push：关闭或挂起页面、连接中断或拒绝浏览器权限时都无法保证送达，各设备之间也不会同步通知或已读状态。
