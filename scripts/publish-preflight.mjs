...
async function verifyNative(manifest) {
  ...
  try {
    ...
    if (nativeManifest.name !== nativePackage || nativeManifest.version !== version || nativeManifest.os?.join(',') !== 'linux' || nativeManifest.cpu?.join(',') !== 'x64') throw new Error('Published native package metadata is incompatible with CFL Linux x64.');
    ...
    const expected = readJson(join(root, 'release', 'codex-artifact.json'));
...
const nativeVersion = await verifyNative(manifest);
...
if (archive) {
  ...
  if (packed.name !== mainPackage || packed.version !== version || packed.optionalDependencies?.[nativePackage] !== nativeVersion) throw new Error('Verified main tarball does not preserve the staged release identity and native pin.');
  if (published) {
...
console.log(JSON.stringify({ package: mainPackage, version, channel, nativeVersion, skip }, null, 2));
