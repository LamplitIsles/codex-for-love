...
type RemoteTts = { provider: 'alibaba' | 'bytedance'; model: string; endpoint: string; credential: string };
...
async function minimax(options: { text: string; voice: string; output: string; signal: AbortSignal; execFileImpl: typeof execFileAsync }): Promise<void> {
  await options.execFileImpl('mmx', ['--non-interactive', 'speech', 'synthesize', '--model', 'speech-2.8-hd', '--text', options.text, '--voice', options.voice, '--format', 'mp3', '--out', options.output], { signal: options.signal, timeout: 60_000, maxBuffer: 64 * 1024 });
